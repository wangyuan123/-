...
日期：2026-09-17。状态：本地核心控制已实施，正式实名平台与运营流程尚待接入。适用场景：中国大陆面向公众提供网络游戏服务。本文保留完整目标设计，当前实际范围和差异以 [实施说明](ANTI_ADDICTION_IMPLEMENTATION.md) 为准。
...
上表记录设计前的差距；旧维护文档中“保留模拟充值”的历史安排不等于公众运营方案。
...
已使用可注入时钟测试边界、同身份并发会话、监护与账号服务，并运行前后端回归。MySQL 专用迁移测试与正式实名平台联调仍待对应环境；详细结果见实施说明。
