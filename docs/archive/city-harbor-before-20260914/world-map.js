...
(function (G) {
  ...
  function MapView(v) {
    ...
    v.innerHTML = '<section class="world-map-shell">' +
      ...
      '<div class="world-map-controls"><button class="world-map-button" data-map="plus" aria-label="放大地图">+</button><button class="world-map-button home" data-map="home">主城</button></div>' +
      ...
      '<div class="world-map-detail" hidden></div></div><div class="world-map-hint">单指拖动 · 双指张开或滚轮向上放大 · 拖动仅浏览，不改变出征起点</div></section>';
    ...
    this.app.view.tabIndex = 0; this.app.view.setAttribute('aria-label', '世界地图，拖动浏览，双指张开或加号按钮放大，也可用方向键浏览');
  ...
  MapView.prototype.draw = function () {
    ...
    this.visible.forEach(function (t) {
      ...
      var pos = c.screen(t.x + .5, t.y + .5), size = Math.min(38, c.scale * .78);
    ...
    if (this.selected) {
      var p = c.screen(this.selected.x + .5, this.selected.y + .5), size = Math.min(42, c.scale * .9);
  ...
  MapView.prototype.bind = function () {
    ...
    this.on(this.shell, 'click', function (e) {
      ...
      else if (action === 'plus') { self.camera.zoom(1.3, self.camera.width/2, self.camera.height/2); self.requestChunks(); self.wake(); }
  ...
  MapView.prototype.renderDetail = function(t) {
    ...
    this.detail.innerHTML='<button class="world-map-detail-close" data-map="close" aria-label="关闭详情">×</button><div class="world-map-detail-head"><img src="'+esc(icon(t))+'" alt=""><div><b>'+esc(name(t))+'</b><div class="world-map-detail-meta">'+esc(meta)+'</div></div></div><p>'+esc(text)+'</p>'+(!t.occupied&&!t.selfCity?'<p>守军和资源情报请通过侦察获取。</p>':'')+'<div class="world-map-actions"></div>';
