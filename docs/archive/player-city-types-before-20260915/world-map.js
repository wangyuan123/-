...
(function (G) {
  ...
  function icon(t, snowCells) {
    if (t.selfCity || t.kind === 'player') return 'img/cities/harbor.webp';
    if (t.kind === 'wild' && t.type === 'snow' && snowCells) return 'img/map/snow-' + G.MapTerrain.snowVariant(t.x,t.y,snowCells) + '.webp';
  ...
  function MapView(v) {
    ...
    this.app.stage.addChild(this.ground, this.groundDetails, this.terrain, this.routes, this.markerLayer, this.captionLayer);
    this.bind();
    this.initMinimap();
    var self = this;
  ...
  MapView.prototype.draw = function () {
    ...
    this.visible.forEach(function (t) {
      ...
      keep.add(key);
      var path = icon(t,snowCells).replace(/\.webp$/, '-map-embedded.png') + '?v=4.4-snow-blend';
  ...
  MapView.prototype.buildMinimapGround = function () {
    ...
    for(var y=0;y<size;y++)for(var x=0;x<size;x++){
      ...
      for(var channel=0;channel<3;channel++)pixels.data[offset+channel]=G.MapOcean?G.MapOcean.paint(channel,land[channel],depth,0):land[channel];
  ...
  MapView.prototype.initMinimap = function () {
    ...
    function move(e){
      ...
      var point=self.elementPoint(canvas,e);
